 ...
 * ⚠️ 중요: 토큰노믹스 독립성 보장
 * ────────────────────────────────
 * 리스테이킹 보상은 20년 TBURN 토큰노믹스와 완전히 분리됩니다.
 * 
 * 보상 출처 분리:
 * - 기존 스테이킹 보상: TBURN 토큰 발행 (20년 토큰노믹스 일정)
 * - 리스테이킹 보상: AVS 운영자가 지불하는 외부 수수료 (USDT, USDC 등)
 * 
 * 리스테이킹 보상은 다음과 같은 특성을 갖습니다:
 * 1. AVS 운영자(Oracle, Bridge, DA Layer 등)가 서비스 이용료로 지불
 * 2. TBURN 토큰 발행량에 영향 없음
 * 3. 스테이블코인 또는 외부 토큰으로 지급
 * 4. 별도 회계 계정(EXTERNAL_AVS_REWARDS)으로 관리
 */
