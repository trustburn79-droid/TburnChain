...
export async function setupVite(app: Express, server: Server) {
  ...
  const serverOptions = {
    middlewareMode: true,
    // Disable HMR entirely in Replit to save memory
    // The Vite client will show "connection lost" messages but this is harmless
    // Memory savings is more important for this enterprise application
...
(async () => {
  ...
  setTimeout(async () => {
    ...
    console.log('[Enterprise] Current TBURN_API_KEY:', process.env.TBURN_API_KEY ? 'configured' : 'not set');
    ...
    console.log('[Enterprise] TBURN enterprise node initialized');
