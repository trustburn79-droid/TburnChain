...
import { Vote, Users, FileText, CheckCircle2, Search, RefreshCw, ArrowLeft, Plus } from "lucide-react";
...
export default function AdminDAOGovernance() {
  ...
  const { data: proposals, isLoading, refetch } = useQuery<{ success: boolean; data: any[] }>({
    ...
    <div className="flex flex-col gap-6 p-6" data-testid="admin-dao-governance-page">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card data-testid="card-total-proposals">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{daoStats.totalProposals}</div>
        ...
        <Card data-testid="card-passed-proposals">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{daoStats.passedProposals}</div>
              ...
              <TableBody>
                {proposalList.map((proposal: any) => (
                  <TableRow key={proposal.id} data-testid={`row-proposal-${proposal.id}`}>
                    ...
                    <TableCell className="text-right text-emerald-500">{proposal.votesFor}</TableCell>
                    <TableCell className="text-right text-red-500">{proposal.votesAgainst}</TableCell>
                    <TableCell>
                      <Badge className={
                    ...
                    <TableCell className="text-muted-foreground">{new Date(proposal.endDate).toLocaleDateString()}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
