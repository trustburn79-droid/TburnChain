...
import { Leaf, Users, DollarSign, Lock, Search, RefreshCw, ArrowLeft, Plus, Eye, Edit, Calendar, Coins, TrendingUp, CheckCircle, FileText, CreditCard } from "lucide-react";
...
const INVESTOR_TYPES = [
  { value: "angel", label: "엔젤 투자자", labelEn: "Angel Investor" },
  ...
  { value: "individual", label: "개인 투자자", labelEn: "Individual" },
...
export default function AdminSeedRound() {
  ...
  const [selectedInvestor, setSelectedInvestor] = useState<SeedInvestor | null>(null);
  ...
  const { data: investorsData, isLoading, refetch } = useQuery<{ success: boolean; data: { investors: SeedInvestor[]; stats: any } }>({
    queryKey: ['/api/admin/token-programs/seed-round/investors'],
  });

  const { data: investorDetailData, isLoading: detailLoading } = useQuery<{ success: boolean; data: { investor: SeedInvestor; payouts: SeedPayout[] } }>({
    queryKey: ['/api/admin/token-programs/seed-round/investors', selectedInvestor?.id],
  ...
  const createInvestorMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      ...
      return apiRequest('POST', '/api/admin/token-programs/seed-round/investors', {
        ...data,
        tokenAmount: tokenAmount.toString(),
        lockedAmount: tokenAmount.toString(),
        distributedAmount: "0",
        status: "pending",
      });
    },
    onSuccess: () => {
      toast({ title: "투자자 등록 완료", description: "새 시드 투자자가 등록되었습니다." });
      queryClient.invalidateQueries({ queryKey: ['/api/admin/token-programs/seed-round/investors'] });
  ...
  const updateInvestorMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<SeedInvestor> }) => {
      return apiRequest('PATCH', `/api/admin/token-programs/seed-round/investors/${id}`, data);
    },
    onSuccess: () => {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      return apiRequest('PATCH', `/api/admin/token-programs/seed-round/investors/${id}`, { status });
    },
    onSuccess: () => {
      toast({ title: "상태 변경 완료", description: "투자자 상태가 변경되었습니다." });
      queryClient.invalidateQueries({ queryKey: ['/api/admin/token-programs/seed-round/investors'] });
      if (selectedInvestor) {
        queryClient.invalidateQueries({ queryKey: ['/api/admin/token-programs/seed-round/investors', selectedInvestor.id] });
      }
    },
  });
  ...
  const addPayoutMutation = useMutation({
    mutationFn: async ({ investorId, data }: { investorId: string; data: typeof payoutForm }) => {
      return apiRequest('POST', `/api/admin/token-programs/seed-round/investors/${investorId}/payouts`, {
        ...data,
        status: "pending",
      });
    },
  ...
  const updatePayoutMutation = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      return apiRequest('PATCH', `/api/admin/token-programs/seed-round/payouts/${id}`, { 
        status,
  ...
  const openEditDialog = (investor: SeedInvestor) => {
  ...
  const openDetailDialog = (investor: SeedInvestor) => {
    setSelectedInvestor(investor);
  ...
  const filteredInvestors = investorList.filter(investor => {
    ...
    const matchesType = typeFilter === "all" || investor.investorType === typeFilter;
    ...
    <div className="flex flex-col gap-6 p-6" data-testid="admin-seed-round-page">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-muted-foreground">Seed Round Management - 1월 3일 정식 오픈 | 토큰 가격: $0.005</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        <Card data-testid="card-total-investors">
          <CardHeader className="flex flex-row items-center justify-between gap-1 pb-2">
            <CardTitle className="text-sm font-medium">시드 투자자</CardTitle>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div>
              <CardTitle>시드 투자자 목록</CardTitle>
              <CardDescription>Seed Investors List - 투자자 관리</CardDescription>
            </div>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p>시드 투자자 데이터가 없습니다</p>
              <p className="text-sm">No seed investors found</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
                  ...
                  <TableRow key={investor.id} data-testid={`row-investor-${investor.id}`}>
                    ...
                    <TableCell>
                      <Badge variant="outline">
                        {INVESTOR_TYPES.find(t => t.value === investor.investorType)?.label || investor.investorType}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell className="text-right">{formatTBURN(investor.tokenAmount)} TBURN</TableCell>
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>새 시드 투자자</DialogTitle>
            <DialogDescription>새로운 시드 라운드 투자자를 등록합니다.</DialogDescription>
          </DialogHeader>
            ...
            <div className="border-t pt-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="grid gap-2">
                  ...
                  <p className="text-xs text-muted-foreground">토큰 가격: $0.005</p>
                </div>
              </div>
            </div>
            <div className="border-t pt-4">
            ...
            <div className="border-t pt-4">
              ...
              <div className="flex gap-6">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <Label htmlFor="saftSigned">SAFT 서명 완료</Label>
                </div>
                <div className="flex items-center gap-2">
                  <Checkbox
                    id="kycVerified"
            ...
            <div className="flex gap-6">
              <div className="flex items-center gap-2">
                <Checkbox
                  ...
                  checked={formData.saftSigned}
                  onCheckedChange={(c) => setFormData({ ...formData, saftSigned: !!c })}
                  data-testid="checkbox-edit-saft"
                />
              ...
              <div className="flex items-center gap-2">
                <Checkbox
                  ...
                  checked={formData.paymentReceived}
                  onCheckedChange={(c) => setFormData({ ...formData, paymentReceived: !!c })}
                  data-testid="checkbox-edit-payment"
                />
          ...
          <DialogHeader>
            <div className="flex items-center justify-between gap-4 flex-wrap">
              <div>
                ...
                <DialogDescription>{selectedInvestor?.company || '시드 투자자'} - 상세 정보 및 지급 관리</DialogDescription>
              </div>
              <div className="flex gap-2">
                <Badge variant="outline">
                  {INVESTOR_TYPES.find(t => t.value === selectedInvestor?.investorType)?.label}
                </Badge>
          ...
          {selectedInvestor && (
            <div className="grid gap-6 py-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  <span className="text-muted-foreground">결제 통화:</span>
                  <p>{selectedInvestor.investmentCurrency.toUpperCase()}</p>
                </div>
                <div>
                  <span className="text-muted-foreground">지갑 주소:</span>
                  <p className="font-mono text-xs">{selectedInvestor.walletAddress || '-'}</p>
                </div>
              </div>
