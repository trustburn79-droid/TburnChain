...
import { KeyRound, Users, DollarSign, Lock, Search, RefreshCw, ArrowLeft, Plus } from "lucide-react";
    ...
    <div className="flex flex-col gap-6 p-6" data-testid="admin-private-round-page">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-muted-foreground">Private Round Management</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card><CardHeader className="flex flex-row items-center justify-between gap-1 pb-2"><CardTitle className="text-sm font-medium">프라이빗 투자자</CardTitle><Users className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold">0</div><p className="text-xs text-muted-foreground">Private Investors</p></CardContent></Card>
        ...
        <Card><CardHeader className="flex flex-row items-center justify-between gap-1 pb-2"><CardTitle className="text-sm font-medium">토큰 가격</CardTitle><KeyRound className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold">$0.01</div><p className="text-xs text-muted-foreground">Token Price</p></CardContent></Card>
        <Card><CardHeader className="flex flex-row items-center justify-between gap-1 pb-2"><CardTitle className="text-sm font-medium">잠금 상태</CardTitle><Lock className="h-4 w-4 text-amber-500" /></CardHeader><CardContent><div className="text-2xl font-bold">6개월</div><p className="text-xs text-muted-foreground">Lock Period</p></CardContent></Card>
      </div>

      <Card>
        <CardHeader>
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div><CardTitle>프라이빗 투자자 목록</CardTitle><CardDescription>Private Investors List</CardDescription></div>
        ...
        <CardContent>
          {isLoading ? (<div className="space-y-2">{[1, 2, 3].map(i => <Skeleton key={i} className="h-12" />)}</div>) : investorList.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground"><KeyRound className="h-12 w-12 mx-auto mb-4 opacity-50" /><p>프라이빗 투자자 데이터가 없습니다</p><p className="text-sm">No private investors found</p></div>
          ) : (
