  ...
  const { data: walletData, isLoading, error, refetch, isFetching } = useQuery<WalletsResponse>({
    ...
    queryFn: async () => {
      ...
      if (Array.isArray(data)) {
        // Apply client-side filtering and pagination for backwards compatibility
        let filtered = data;
        
        if (searchInput) {
          const searchLower = searchInput.toLowerCase();
          filtered = filtered.filter(w => w.address.toLowerCase().includes(searchLower));
        }
        
        if (selectedBalanceFilter !== "all") {
        ...
        if (selectedActivityFilter !== "all") {
          ...
          filtered = filtered.filter(w => {
            ...
            return selectedActivityFilter === "active" ? lastTx > thirtyDaysAgo : lastTx <= thirtyDaysAgo;
          });
        }
        
        if (selectedStakingFilter !== "all") {
          filtered = filtered.filter(w => {
            const isStaking = parseFloat(w.stakedBalance) > 0;
            return selectedStakingFilter === "staking" ? isStaking : !isStaking;
          });
        }
        ...
        const paginatedWallets = filtered.slice(offset, offset + pageSize);
