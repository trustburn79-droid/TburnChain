...
export default function NetworkStatus() {
  ...
  const services = [
    { name: t('publicPages.network.status.services.mainnetRpc.name'), desc: t('publicPages.network.status.services.mainnetRpc.desc'), uptime: "99.99%" },
    { name: t('publicPages.network.status.services.trustScore.name'), desc: t('publicPages.network.status.services.trustScore.desc'), uptime: "99.98%" },
    { name: t('publicPages.network.status.services.crossChainBridge.name'), desc: t('publicPages.network.status.services.crossChainBridge.desc'), uptime: "99.92%" },
    { name: t('publicPages.network.status.services.quantumSecurity.name'), desc: t('publicPages.network.status.services.quantumSecurity.desc'), uptime: "100%" },
  ];
    ...
    <main className="flex-grow relative z-10 bg-gray-50 dark:bg-transparent transition-colors">
      <section className="relative py-24 px-6 overflow-hidden border-b border-gray-200 dark:border-white/5">
        ...
        <div className="container mx-auto max-w-5xl relative z-10">
          ...
          >
            <RefreshCw className="w-4 h-4" /> {t('publicPages.network.status.buttons.refreshData')}
          </button>
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="spotlight-card rounded-xl p-6 border border-gray-300 dark:border-white/10" data-testid="card-ai-strategic">
              ...
              <div className="flex justify-between text-sm mt-4">
                <span className="text-gray-500">{t('publicPages.network.status.ai.latency')}</span>
            ...
            {services.map((service) => (
              <div key={service.name} className="spotlight-card rounded-lg p-4 flex items-center justify-between border border-gray-300 dark:border-white/10">
                ...
                <div className="text-right">
                  <span className="text-[#00ff9d] text-xs font-bold block">{t('publicPages.network.status.operational')}</span>
        ...
        <div className="container mx-auto max-w-7xl">
          ...
          <div className="spotlight-card rounded-xl p-6 border border-gray-300 dark:border-white/10">
            ...
            <div className="flex justify-between text-xs text-gray-500 mt-2">
              <span>{t('publicPages.network.status.daysAgo')}</span>
              <span>{t('publicPages.network.status.today')}</span>
            </div>
          </div>
        </div>
      </section>
