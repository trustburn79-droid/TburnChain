...
const mockHistoricalData: HistoricalData[] = [
  { date: "12/29", avgLatency: 15, throughput: 12500, successRate: 99.92 },
  { date: "12/30", avgLatency: 14, throughput: 13200, successRate: 99.95 },
  { date: "12/31", avgLatency: 16, throughput: 12800, successRate: 99.90 },
  { date: "01/01", avgLatency: 13, throughput: 14100, successRate: 99.96 },
  { date: "01/02", avgLatency: 14, throughput: 13800, successRate: 99.94 },
  { date: "01/03", avgLatency: 15, throughput: 13400, successRate: 99.93 },
  { date: "01/04", avgLatency: 14, throughput: 13790, successRate: 99.95 },
];

export default function TestnetRpcBenchmark() {
  ...
  const runBenchmark = useCallback(async () => {
    ...
    for (let i = 0; i < totalIterations; i++) {
      ...
      try {
        await fetch('/api/public/v1/testnet/stats', { signal });
  ...
  const getLatencyColor = (latency: number) => {
    if (latency < 15) return '#00ff9d';
    if (latency < 30) return '#ffd700';
    if (latency < 60) return '#ff6b35';
    return '#ff0055';
  };
  ...
  return (
    <main className="flex-grow relative z-10 bg-gray-50 dark:bg-transparent transition-colors">
      <section className="relative py-8 overflow-hidden border-b border-gray-200 dark:border-white/5">
        <div className="absolute top-0 right-1/3 w-[600px] h-[500px] bg-yellow-500/10 blur-[120px] rounded-full pointer-events-none" />
              ...
              <div className="grid md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    min="10"
                    max="1000"
                    data-testid="input-testnet-iterations"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    min="1"
                    max="100"
                    data-testid="input-testnet-concurrency"
                  />
                </div>
                ...
                <div className="space-y-4">
                  {results.map((result) => (
                    <div 
                      ...
                      data-testid={`testnet-result-${result.id}`}
                    >
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                          <code className="text-sm font-mono text-yellow-500">{result.method}</code>
                  ...
                  <div className="h-32 flex items-end gap-2">
                    {mockHistoricalData.map((data, i) => (
                      <div key={i} className="flex-1 flex flex-col items-center gap-1">
                        <div 
                          className="w-full rounded-t bg-gradient-to-t from-yellow-500 to-yellow-400 transition-all"
                          style={{ height: `${(data.avgLatency / 20) * 100}%` }}
                        />
                    ...
                    {mockHistoricalData.map((data, i) => (
                      <div key={i} className="flex-1 flex flex-col items-center gap-1">
                        <div 
                          ...
                          style={{ height: `${(data.throughput / 15000) * 100}%` }}
                        />
                        <span className="text-[9px] text-gray-500">{data.date}</span>
                      </div>
                    ))}
