...
import { DEFAULT_CONFIG, CHAIN_CONSTANTS, GENESIS_VALIDATORS_REGIONS } from './config/default';
...
program
  ...
  .action(async () => {
    ...
    try {
      ...
      GENESIS_VALIDATORS_REGIONS.forEach((r, i) => {
        console.log(`  ${i + 1}. ${r.region} (${r.datacenter})`);
      });
      const regionInput = await prompt(rl, 'Enter region number (1-7) [default: 1]: ') || '1';
      const regionIndex = Math.max(0, Math.min(parseInt(regionInput) - 1, GENESIS_VALIDATORS_REGIONS.length - 1));
      const selectedRegion = GENESIS_VALIDATORS_REGIONS[regionIndex] || GENESIS_VALIDATORS_REGIONS[0];
      ...
      let keyPair: { privateKey: string; publicKey: string; address: string };
...
program
  ...
  .action(async (options) => {
    ...
    console.log('╠══════════════════════════════════════════════════════════════╣');
    console.log(`║  Chain ID: ${CHAIN_CONSTANTS.CHAIN_ID}                                                ║`);
    console.log(`║  Network: ${CHAIN_CONSTANTS.NETWORK_ID}                                    ║`);
    console.log(`║  Block Time: ${CHAIN_CONSTANTS.BLOCK_TIME_MS}ms                                           ║`);
    console.log('╚══════════════════════════════════════════════════════════════╝');
    console.log('');
    ...
    try {
      ...
      console.error(`❌ Failed to start validator node: ${(error as Error).message}`);
      process.exit(1);
    }
  });
...
program
  ...
  .action(async (action: string, options: { config: string; privateKey?: string; output?: string; input?: string }) => {
    switch (action) {
      case 'generate': {
        const keyPair = CryptoManager.generateKeyPair();
      ...
      case 'import': {
        ...
        try {
          ...
          console.error(`❌ Invalid private key: ${(error as Error).message}`);
          process.exit(1);
        }
        break;
      }
...
program
  ...
  .action(async (options: { url: string }) => {
    try {
      ...
      const status = await response.json() as NodeStatusResponse;
      ...
      console.error(`❌ Failed to connect to validator: ${(error as Error).message}`);
...
program
  ...
  .action(() => {
    ...
    for (const region of GENESIS_VALIDATORS_REGIONS) {
    ...
    console.log('');
    console.log(`Total Genesis Validators: ${CHAIN_CONSTANTS.MAX_VALIDATORS}`);
  });
