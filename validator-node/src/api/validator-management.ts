...
const log = createModuleLogger('ValidatorManagement');
...
const validators: Map<string, ValidatorRegistration> = new Map();
const infrastructureStatus: Map<string, InfrastructureStatus> = new Map();
...
export function createValidatorManagementRouter(): Router {
  const router = Router();
  ...
  router.get('/validators', async (_req: Request, res: Response) => {
    try {
      ...
      log.error('Failed to get validators', { error: (error as Error).message });
      res.status(500).json({ success: false, error: 'Internal server error' });
    }
  });

  // Register new validator (application)
  router.post('/validators/register', express.json(), async (req: Request, res: Response) => {
    try {
      ...
      log.error('Failed to register validator', { error: (error as Error).message });
      res.status(500).json({ success: false, error: 'Internal server error' });
    }
  });
  ...
  router.get('/validators/:id', async (req: Request, res: Response) => {
    try {
      ...
      log.error('Failed to get validator', { error: (error as Error).message });
      res.status(500).json({ success: false, error: 'Internal server error' });
    }
  });

  // Update validator info
  router.patch('/validators/:id', express.json(), async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const validator = validators.get(id);
      ...
      for (const key of Object.keys(updates)) {
        if (allowedUpdates.includes(key)) {
          (validator as any)[key] = updates[key];
        }
      }
      ...
      log.error('Failed to update validator', { error: (error as Error).message });
      res.status(500).json({ success: false, error: 'Internal server error' });
    }
  });
  ...
  router.get('/infrastructure', async (_req: Request, res: Response) => {
    try {
      ...
      log.error('Failed to get infrastructure', { error: (error as Error).message });
      res.status(500).json({ success: false, error: 'Internal server error' });
    }
  });

  router.get('/infrastructure/:validatorId', async (req: Request, res: Response) => {
    try {
      ...
      log.error('Failed to get infrastructure status', { error: (error as Error).message });
      res.status(500).json({ success: false, error: 'Internal server error' });
    }
  });

  // Heartbeat/status update from validator node
  router.post('/infrastructure/:validatorId/heartbeat', express.json(), async (req: Request, res: Response) => {
    try {
      const { validatorId } = req.params;
      ...
      const infraStatus: InfrastructureStatus = {
        validatorId,
      ...
      log.error('Failed to process heartbeat', { error: (error as Error).message });
      res.status(500).json({ success: false, error: 'Internal server error' });
    }
  });
  ...
  router.get('/governance/proposals', async (req: Request, res: Response) => {
    try {
      ...
      log.error('Failed to get proposals', { error: (error as Error).message });
      res.status(500).json({ success: false, error: 'Internal server error' });
    }
  });
  ...
  router.get('/governance/proposals/:id', async (req: Request, res: Response) => {
    try {
      ...
      log.error('Failed to get proposal', { error: (error as Error).message });
      res.status(500).json({ success: false, error: 'Internal server error' });
    }
  });

  // Submit vote
  router.post('/governance/proposals/:id/vote', express.json(), async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      ...
      log.error('Failed to submit vote', { error: (error as Error).message });
      res.status(500).json({ success: false, error: 'Internal server error' });
    }
  });

  // Get validator's voting history
  router.get('/governance/validators/:validatorId/votes', async (req: Request, res: Response) => {
    try {
      const { validatorId } = req.params;
      const validatorVotes = votes.get(validatorId) || [];
      ...
      log.error('Failed to get voting history', { error: (error as Error).message });
      res.status(500).json({ success: false, error: 'Internal server error' });
    }
  });
