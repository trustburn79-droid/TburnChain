...
- **Unified TPS Synchronization (Dec 24 Launch)**: CRITICAL LEGAL REQUIREMENT - All API endpoints display synchronized TPS from shared 2-second shard cache. Implementation: (1) `TBurnEnterpriseNode.generateShards()` uses 2s cache (`shardsCache`) for exact TPS sync; (2) `/api/network/stats`, `/api/shards`, `/api/sharding` all call `generateShards()` for identical TPS values; (3) `calculateRealTimeTps()` in `routes.ts` uses `generateShards()` for sync; (4) `getUnifiedTpsData()` in `public-api-routes.ts` sums shard TPS; (5) No separate network stats cache - always fresh from shards cache; (6) Base TPS formula: 64 shards × 625 tx × 0.525 load × 10 blocks/sec ≈ 210,000 TPS; (7) Synchronized routes: /admin, /admin/shards, /app, /scan, /rpc, /vc; (8) Race condition at cache boundary (2s) is acceptable for real-time blockchain system.
