...
- **External Validator Registration System (2026-01-15)**: Production-ready 3-step registration wizard for external validators at `/validator-registration`. Features: Cryptographic proof verification (step 1), node information collection with tier selection (step 2), infrastructure documentation (step 3). Validator tiers: Community (100+ TBURN), Standard (10K+ TBURN), Pioneer (100K+ TBURN, requires 2-of-3 multi-sig), Genesis (1M+ TBURN, requires 3-of-3 multi-sig). API endpoint: `POST /api/external-validators/register` (public, no session required). Stake amounts converted to WEI format (multiply by 10^18). Valid regions: global, us-east, us-west, eu-west, eu-central, asia-east, asia-south, asia-southeast, oceania, south-america, africa. Registration workflow: pending → under_review → approved (API key issued via email) OR rejected. API key format: `vk_live_<64-char-hex>` with bcrypt+pepper hashing. Files: `client/src/pages/validator-registration.tsx`, `server/routes/external-validator-routes.ts`, `server/core/validators/enterprise-external-validator-engine.ts`.
