...
- **Unified TPS Synchronization (Dec 23 Launch)**: CRITICAL LEGAL REQUIREMENT - All 11 routes display synchronized TPS from Enterprise Node (`/api/network/stats`). Base TPS formula: 64 shards × 625 tx × 0.525 load × 10 blocks/sec ≈ 210,000 TPS. Implementation: (1) All TPS calculations are deterministic (no Math.random()) for legal compliance; (2) `getUnifiedTpsData()` helper in `server/routes/public-api-routes.ts` provides Enterprise Node data; (3) All admin pages (shards, performance, dashboard-builder, realtime, metrics-explorer, sla) fetch from `/api/network/stats`; (4) Fallback values use deterministic sine wave variations (±2.5% of base TPS); (5) Pages block rendering until Enterprise Node data arrives to prevent 210K fallback display; (6) 11 synchronized routes: /admin, /admin/shards, /admin/performance, /admin/dashboard-builder, /admin/realtime, /admin/metrics-explorer, /admin/sla, /app, /scan, /rpc, /vc.
- **TBurn Logo Branding System**: Unified SVG-based TBurnLogo component (`client/src/components/tburn-logo.tsx`) applied across all application pages. Features: gradient/solid color options via `symbolColor` prop, customizable `fontSize`, optional `showText` prop for T letter display, automatic text color contrast. Applied locations: PublicHeader, PublicFooter, favicon, loading screen, /login header, /app sidebar, ScanLayout (/scan pages header/footer), VCLayout (/vc pages header/footer), /admin/appearance showcase (25+ color combinations), /qna header, /user sidebar, /token-generator sidebar, /nft-marketplace-standalone sidebar, /tree header. TestnetScan uses FlaskConical icon intentionally to distinguish testnet environment.
