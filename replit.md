...
- **TBurn Logo Branding System**: Unified SVG-based TBurnLogo component (`client/src/components/tburn-logo.tsx`) applied across all application pages. Features: gradient/solid color options via `symbolColor` prop, customizable `fontSize`, optional `showText` prop for T letter display, automatic text color contrast. Applied locations: PublicHeader, PublicFooter, favicon, loading screen, /login header, /app sidebar, ScanLayout (/scan pages header/footer), VCLayout (/vc pages header/footer), /admin/appearance showcase (25+ color combinations). TestnetScan uses FlaskConical icon intentionally to distinguish testnet environment.
