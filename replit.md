...
- **Instant First Load Fix (v7.6)**: CRITICAL production cold-start optimization. SPA fallback (`index.html`) is now registered BEFORE heavy backend initialization, enabling instant page load on first visit. (1) `server/index-prod.ts` Phase 2 registers SPA fallback immediately after static file serving. (2) API routes return 503 with `retryAfter: 5` while services initialize. (3) `queryClient.ts` implements `ServiceInitializingError` class with automatic 5-second retry (up to 12 attempts = 60 seconds). (4) Users see the React app instantly; API data loads automatically when backend is ready.
